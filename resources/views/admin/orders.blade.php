...
<table class="table table-hover">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>£ {{$order->price}}</td>
            <td>
                <form id="{{'orderedit-'.$order->id}}" action="{{ route('admin.editorder',$order->id)}}" method="post">
                    @csrf @method('put')
            ...
            <td>
                <span class="pointer mr-3" title="View Details" data-toggle="modal" data-target="#orderDetail"
                    data-billdetail="{{ $order->billdetail }}" data-prisonerdetail="{{ $order->prisonerdetail }}"><i
                ...
                <form style="display: none" id="{{'order-'.$order->id}}" action="{{ route('order.delete',$order->id)}}"
                    method="post">
                    @csrf @method('delete')
                </form>
            </td>
